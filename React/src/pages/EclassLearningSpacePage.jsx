...
export default function EclassLearningSpacePage() {
  ...
  const [activeTab, setActiveTab] = useState("assignment"); // 'assignment' or 'material'
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      const responseData = await myLecture.myAssignmentListByLecNo(
        params.lectureName
      );
      setAssignmentList(responseData);

      // 자료실 데이터도 가져오기 (실제 API 호출로 변경 필요)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      console.log("params.lectureName:", params.lectureName);
  ...
  const renderAssignmentContent = () => (
    <>
      <div className="flex items-center justify-between mb-8">
        ...
        {authority === "ROLE_PROF" && (
          <Link
            to={`/eclass/lecture/${user.username}/${params.lectureName}/assignment/new`}
            state={lecture}
            className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-xl"
          >
            <svg
              className="w-5 h-5"
              fill="none"
      ...
      <div className="grid gap-4 mb-8">
        ...
        {currentData.map((a) => (
          <div
            ...
            onClick={() =>
              navigate(
                `/eclass/lecture/${params.userNo}/${params.lectureName}/assignment/${a.assignTitle}`,
                {
                  state: { ...a },
                }
              )
  ...
  return (
    <div>
      <PageMeta title="부엉대 | 사이버 학습실" />
      <PageBreadcrumb pageTitle="사이버 학습실" breadcrumbs={breadcrumbs} />
      ...
      <div className="rounded-2xl border border-gray-200 bg-white px-5 py-7 xl:px-10 xl:py-12">
        ...
        {activeTab === "material" && <MaterialRoom lecture={lecture} />}
        {activeTab === "exam" && <ExamRoom />}
      </div>
    </div>
  );
}
