 ...
 * 수정일			수정자	수정내용
 * ========================================
 * 2025-07-22 	김태수	최초 생성
--%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="jakarta.tags.core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %> <%-- 날짜 포맷을 위해 fmt 태그 추가 --%>
		...
		<table class="defaultTable">
			<thead class="tableHead">
				<tr>
					...
					<th class="tableTh">논문 번호</th> <%-- 컬럼명에 맞춰 수정 --%>
					<th class="tableTh">제목</th>
					<th class="tableTh">등록일시</th>
					<th class="tableTh">교수 이름</th> <%-- 컬럼명에 맞춰 수정 --%>
					<th class="tableTh">처리 현황</th>
					<%-- 이전 JSP에 있던 '파일', '관리' 컬럼은 쿼리 결과에 없으므로 제거합니다. --%>
				</tr>
			</thead>
			<tbody>
				<c:choose>
					<c:when test="${not empty thesisList }">
						<c:forEach items="${thesisList }" var="thesis" varStatus="status">
							<c:url value="/staff/thesis/thesisDetail.do" var="detailURL">
							...
							<tr onclick="location.href='${detailURL}'" class="tableRowHover tableRowStripe">
								...
								<td class="tableTd">${thesis.thesisNo}</td> 
					...
					<c:otherwise>
						<tr class="tableRowHover tableRowStripe">
							<td class="tableTd" colspan="6">등록한 논문 목록이 존재하지 않습니다.</td> <%-- colspan 갯수 조정 --%>
						</tr>
					</c:otherwise>
				</c:choose>
			</tbody>
