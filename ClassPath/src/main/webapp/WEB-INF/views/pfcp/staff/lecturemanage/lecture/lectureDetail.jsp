...
<link rel="stylesheet" href="/dist/assets/css/test/lectureDetail2.css">
...
<div class="lectureDetail">
	...
	<div class="sectionHeaderLine">
		<div>
			<div class="sectionHeaderTitle">[
				${lectureStudent[0].lectureReq.lecName} ] 수강 학생 목록</div>
			...
			<tbody>
				...
				<c:choose>
					<c:when test="${not empty lectureStudent }">
						<c:forEach items="${lectureStudent }" var="lectureStudent"
							varStatus="status">
							<tr class="tableRowHover" onclick="moveToAttendance(this)"
								data-user-no="${lectureStudent.user.userNo}"
								data-enr-no="${lectureStudent.enrollNo}">
								<td class="tableTd">${status.index+1}</td>
								<td class="tableTd">${lectureStudent.department.departmentName }</td>
								<td class="tableTd">${lectureStudent.student.studentGrade }</td>
					...
					<c:otherwise>
						<tr>
							<td colspan="7" class="tableTd noticeInfo">해당 강의를 수강하는 학생이
								없음.</td>
						</tr>
					</c:otherwise>
				</c:choose>
	...
	<div class="pagination">
		<c:if test="${paging.currentPageNo > 10}">
			<a
				href="?page=${paging.firstPageNoOnPageList - paging.pageSize}&keyword=${keyword}"
		...
		<c:forEach var="i" begin="${paging.firstPageNoOnPageList}"
			...
			<c:choose>
				...
				<c:otherwise>
					<a
						href="?page=${i}&keyword=${keyword}&searchType=${searchType}&what=${lectureStudent[0].lecNo}"
		...
		<c:if test="${paging.currentPageNo < paging.totalPageCount}">
			<c:if test="${paging.totalPageCount} > 10">
				<a
					href="?page=${paging.firstPageNoOnPageList + paging.pageSize}&keyword=${keyword}"
