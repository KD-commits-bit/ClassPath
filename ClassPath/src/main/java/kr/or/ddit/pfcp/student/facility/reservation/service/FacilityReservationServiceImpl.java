  ...
  public List<String> getReservedTimesByFacility(String facilityNo) {
	  DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
	    ...
	    return reservations.stream()
	        .map(FacilityReservationVO::getPreferredDate)
	        .map(dateStr -> {
	            LocalDateTime dt = LocalDateTime.parse(dateStr, formatter);
	            String day = dt.getDayOfWeek().name().substring(0, 3); // MON, TUE, ...
	            return day + "_" + dt.getHour(); // 예: MON_9
	        })
	        .collect(Collectors.toList());
  }
  
  public boolean cancelReservation(String reservationNo, String userNo) {
      // 유저가 이 예약의 주인인지 확인
      FacilityReservationVO reservation = facilityReservationMapper.selectReservationByNo(reservationNo);
      log.info("reservation : {}",reservation);
      if (reservation != null && userNo.equals(reservation.getUserNo())) {
          return facilityReservationMapper.deleteReservation(reservationNo) > 0;
      }
      return false;
  }
	...
	public List<FacilityReservationVO> readFacilityReservationByUserNo(String userNo) {
		return facilityReservationMapper.selectReservationByUserNo(userNo);
	}
