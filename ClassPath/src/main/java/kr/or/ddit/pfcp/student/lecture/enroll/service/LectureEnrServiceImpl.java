...
public class LectureEnrServiceImpl implements LectureEnrService {
  ...
  public List<LectureEnrVO> retrieveLectureEnrByUserNo(String studentNo) {
	...
	@Override
	public List<LectureVO> readLectureList(String userNo, String departmentNo, String day, String period, Integer credit, String lecName) {
		return lectureEnrMapper.selectLectureListBySemester(userNo, departmentNo, day, period, credit, lecName);
	}
	...
	private Set<String> toTimeBlocks(LectureReqVO req) {
	    ...
	    for (String day : days) {
	        ...
	        for (String time : times) {
	            // 시간 문자열에서 숫자만 추출: ex) "1교시" → "1"
	            String cleanTime = time.trim().replaceAll("[^0-9]", "");
	            if (!cleanTime.isEmpty()) {
	                blocks.add(cleanDay + cleanTime); // 예: "월1", "화2"
	            }
	        }
	    }
