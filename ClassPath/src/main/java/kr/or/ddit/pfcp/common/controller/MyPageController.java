...
public class MyPageController {
	...
	public String mypage(Authentication auth, Model model) {
	    ...
	    UserVO user;
	    UserVOWrapper useruser = (UserVOWrapper) auth.getPrincipal();
	    
	    log.info("departmentName : {}", useruser.getRealUser()); 
	...
	public void serveProfileImage(
	  ...
	  @GetMapping("/mypage/updateForm.do")
	  public String updateForm(
	        ...
	        if(username.startsWith("ST")) {
	          user = myPageSerivce.readMyPageST(username);
	        } else if(username.startsWith("PR")) {
	          user = myPageSerivce.readMyPagePr(username);
	        } else {
	          user = myPageSerivce.readMyPageAD(username);
	        }
	        model.addAttribute(MODELNAME, user);
	        return "user/userUpdateForm";
	 }
	  
	  @PostMapping("/mypage/updateProcess.do")
	  public String updateProcess(
	        @Valid @ModelAttribute(MODELNAME) UserVO user,
	        BindingResult errors,
	    ...
	    if(errors.hasErrors()) {
	      ...
	      redirectAttributes.addFlashAttribute(customErrors);
	      lvn = "redirect:/mypage/updateForm.do";
	    } else {
	      try {
	        myPageSerivce.updateMyPage(user);
	        lvn = "redirect:/mypage";
